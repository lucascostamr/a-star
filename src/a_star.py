...
class AStar:
    ...
    def start(self, tabuleiro: list[str]):
        ...
        while True:
                ...
                if self.movimento_valido(nova_coordenada, tabuleiro, self.fruta_coletada):
                    ...
                    if tabuleiro[nova_coordenada[0]][nova_coordenada[1]] == "A":
                        distancia_percorrida = (
                            distancia_diagonal[distancia]
                            + self.menor_caminho.distancia_percorrida
                            + 1
                        )
                    else:
                        distancia_percorrida = (
                    ...
                    funcao_heuristica = self.cacular_funcao_heuristica(
                    ...
                    caminho_percorrido = [
                        *self.menor_caminho.caminho_percorrido,
                        nova_coordenada,
                    ]
            ...
            self.menor_caminho: Caminho = self.retorna_menor_caminho(self.lista_caminhos)
            coordenadas_personagem = self.menor_caminho.coordenada

            if self.menor_caminho.coordenada == coordenadas_chegada or self.lista_caminhos is None:
                break
        return self.menor_caminho
    ...
    def cacular_funcao_heuristica(
        ...
        hipotenusa = hypot(abs(x_personagem - x_chegada),
                           abs(y_personagem - y_chegada))

        return hipotenusa + distancia_percorrida

    def movimento_valido(self, coordenadas, tabuleiro, fruta_coletada):
        num_linhas = len(tabuleiro)
        num_coluas = len(tabuleiro[0])
    ...
    def retorna_menor_caminho(self, lista_caminhos) -> Caminho:
